...
import type { Course, CourseWithRating } from "@/types/domain"
  ...
  return (
    <Card className="group overflow-hidden transition duration-300 hover:-translate-y-1 hover:border-primary/30 hover:shadow-xl">
      <Link href={`/courses/${course.id}`} className="block focus-visible:ring-inset">
        <div className="relative flex aspect-[16/9] items-center justify-center overflow-hidden bg-gradient-to-br from-[#dcecf8] to-[#b8d7ee]">
          {course.imageUrl ? (
            // The backend owns the dynamic MinIO host, so a native image is intentional here.
        ...
        <CardContent className="space-y-3 p-5">
          <div className="flex items-start justify-between gap-3"><h3 className="line-clamp-2 text-lg font-semibold leading-snug">{course.title}</h3><ArrowUpRight className="mt-1 size-5 shrink-0 text-primary transition group-hover:translate-x-0.5 group-hover:-translate-y-0.5" /></div>
          ...
          {rating !== null && <div className="flex items-center gap-1.5 text-sm font-semibold text-foreground"><Star className="size-4 fill-amber-400 text-amber-400" />{rating.toFixed(1)}<span className="font-normal text-muted-foreground">/ 10</span></div>}
        </CardContent>
      </Link>
    </Card>
  )
