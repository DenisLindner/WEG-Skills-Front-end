...
export function SiteHeader({ session, user }: HeaderProps) {
  return (
    <header className="sticky top-0 z-50 border-b border-border/70 bg-white/90 backdrop-blur-xl">
      <div className="content-grid flex h-16 items-center justify-between gap-4">
        <Logo />
        <nav className="hidden items-center lg:flex" aria-label="Navegação principal"><Navigation session={session} /></nav>
        ...
        <details className="group relative lg:hidden">
          <summary className="flex size-10 cursor-pointer list-none items-center justify-center rounded-xl border border-border bg-white text-primary"><Menu className="size-5 group-open:hidden" /><ChevronDown className="hidden size-5 group-open:block" /><span className="sr-only">Abrir menu</span></summary>
          <div className="absolute right-0 mt-3 flex w-72 flex-col gap-1 rounded-2xl border border-border bg-white p-3 shadow-xl">
            ...
            {session.authenticated ? (
              <><Link href="/profile" className="flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium"><UserRound className="size-4" />{user?.name ?? "Meu perfil"}</Link><LogoutButton compact /></>
            ) : (
              <><Link href="/login" className={buttonVariants({ variant: "outline", size: "sm" })}>Entrar</Link><Link href="/register" className={buttonVariants({ size: "sm" })}>Criar conta</Link></>
            )}
          </div>
        </details>
      </div>
